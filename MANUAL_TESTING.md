...
- A test wallet funded with SAFE tokens and ETH for gas.
- A validator address registered on the staking contract to delegate to.
- For rewards testing: `VITE_MERKLE_DROP_ADDRESS` configured, and a proof file generated for
  your test address via `scripts/merkle-config.json` + `yarn generate:proofs` (see the
  "Merkle Proof Generation" section of `CLAUDE.md`), uploaded to wherever
  `VITE_REWARDS_BASE_URL` points.

How to reach each connection mode:

- **Browser wallet**: open the app directly with MetaMask (or similar) installed.
- **WalletConnect**: requires `VITE_WALLETCONNECT_PROJECT_ID` set; open the app, click Connect
  Wallet → WalletConnect, and pair with a mobile wallet (or a second desktop wallet) by
  scanning the QR code / following the URI.
- **Safe App**: open [Safe{Wallet}](https://app.safe.global), go to Apps → "My custom apps",
  add the deployed app URL, and open it from inside the Safe.
